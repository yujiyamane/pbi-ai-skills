...
Default template: `pbi-user-guide-drafter/assets/template.docx`
...
python pbi-user-guide-drafter/scripts/write_docx.py \
  ...
  --output "<pbip_dir>/User Guide - <Name> Dashboard (Power BI User Guide).docx" \
...
The `assets/template.docx` file must be present (or pass `--template <custom_path>`).
...
| Missing `assets/template.docx` | Template not found — check assets/ directory or pass `--template` |
